...
class Strategy01PeakFlowExitTests(unittest.TestCase):
    ...
    def test_s02_profit_protection_is_permanently_configured(self):
        profile = STRATEGY_PROFILES[
            StrategyId.S02_LOW_BUY_SELL_EXHAUSTION
        ]
        self.assertEqual(profile.supported_weak_peak_confirm_sec, 3)
        self.assertEqual(profile.supported_weak_peak_active_date, "*")
        self.assertEqual(profile.supported_weak_peak_arm_return_pct, Decimal("5"))
        self.assertEqual(profile.supported_weak_peak_drop_pct, Decimal("1.2"))
        self.assertEqual(profile.supported_weak_peak_score, 1)

    def test_s02_profit_protection_waits_for_rising_hold_to_break(self):
        state = self.state(strategy=StrategyId.S02_LOW_BUY_SELL_EXHAUSTION)
        held = self.engine.evaluate(
            state, self.obs(0, "106", daily_ma_permit=True))
        still_held = self.engine.evaluate(
            state, self.obs(1, "104.60", daily_ma_permit=True))
        watch = self.engine.evaluate(
            state, self.obs(2, "104.60", daily_ma_permit=False))
        sold = self.engine.evaluate(
            state, self.obs(6, "104.60", daily_ma_permit=False))

        self.assertEqual(held.action, HoldSellAction.HOLD)
        self.assertEqual(still_held.action, HoldSellAction.HOLD)
        self.assertIsNotNone(watch)
        self.assertEqual(watch.action, HoldSellAction.WATCH)
        self.assertEqual(sold.action, HoldSellAction.SELL)
        self.assertIn("S02_PEAK_5_DROP_1P2_FLOW_1OF4_EXIT", sold.reason)
    ...
    def test_s01_ma5_trend_hold_rejects_price_equal_to_ma5(self):
        self.engine = UnifiedHoldSellEngine(
            HoldSellConfig(s01_ma5_trend_hold_active_date="*"))
        state = self.state()
        self.engine.evaluate(
        ...
        watch = self.engine.evaluate(
            ...
            self.obs(
                ...
                ma5_rising=True,
                ma10_support=True,
                ma5_value=Decimal("102.70"),
            ),
        )
        self.assertEqual(watch.action, HoldSellAction.WATCH)
        self.assertNotEqual(watch.reason, "S01_MA5_TREND_HOLD")

    def test_hard_stop_fires_even_with_ma5_trend_fields_true(self):
        # 하드손절은 규칙 사슬 맨 앞 — 추세보유 관측값이 전부 참이어도
        # (보류는 수익>0 전제라 애초에 성립 불가) 손절이 그대로 나가야 한다.
        self.engine = UnifiedHoldSellEngine(
            HoldSellConfig(s01_ma5_trend_hold_active_date="*"))
        state = self.state()
        sold = self.engine.evaluate(
        ...
        self.assertEqual(sold.action, HoldSellAction.EMERGENCY_SELL)
        self.assertIn("HARD_STOP", sold.reason)

    def test_time_exit_overrides_s01_ma5_trend_hold(self):
        # 15:10 시간청산도 규칙 사슬에서 보류보다 앞 — 보류 조건이 전부 참이어도
        # 마감 청산이 먼저 팔아야 한다.
        self.engine = UnifiedHoldSellEngine(
            HoldSellConfig(s01_ma5_trend_hold_active_date="*"))
        state = self.state()
        sold = self.engine.evaluate(
            state,
            self.obs(
                5 * 3600 + 11 * 60,  # 10:00 기준 +5h11m = 15:11 KST
                "102.70",
                price_above_ma5=True,
                ma5_rising=True,
                ma10_support=True,
        ...
        self.assertEqual(sold.action, HoldSellAction.EMERGENCY_SELL)
        self.assertIn("TIME_EXIT", sold.reason)

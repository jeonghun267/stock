...
from strategy_01_rotation_engine_v2 import Strategy01Engine, kst_now
...
class Strategy02Tests(unittest.TestCase):
    ...
    def test_missing_high_range_metadata_fails_closed(self) -> None:
        with tempfile.TemporaryDirectory() as folder:
            ...
            config.watch_path.write_text(json.dumps({
                "for_date": self.now.strftime("%Y%m%d"),
                "codes": ["123456"],
                "all_meta": {},
            }), encoding="utf-8")
            ...
            points, status, watch_count, range_meta = load_live_points(
                config, self.now.replace(tzinfo=None)
            )
    ...
    def test_s02_arrival_price_collar_blocks_only_upward_chase(self) -> None:
        with tempfile.TemporaryDirectory() as folder:
            ...
            engine.config = replace(build_config(), snapshot_path=snapshot_path)
            engine._s02_arrival_max_chase_bps = 25.0
            engine._s02_signal_rows_by_code = {
                "123456": {"code": "123456", "name": "TEST", "price": 10_000}
            }
            events = []
            engine._event = lambda event, **fields: events.append((event, fields))

            with patch.object(
                Strategy01Engine, "_snapshot_point",
                return_value={"price": 10_030},
            ):
                blocked = engine._snapshot_point("123456", self.now)
            self.assertIsNone(blocked)
            self.assertIn("ARRIVAL_PRICE_COLLAR", events[-1][1]["reason"])

            events.clear()
            with patch.object(
                Strategy01Engine, "_snapshot_point",
                return_value={"price": 10_020},
            ):
                allowed = engine._snapshot_point("123456", self.now)
            self.assertEqual(allowed["price"], 10_020)
            self.assertEqual(
                engine._s02_signal_rows_by_code["123456"]["arrival_chase_bps"],
                20.0,
            )
    ...
    def test_s02_signal_routes_to_common_rotation_engine(self) -> None:
        with tempfile.TemporaryDirectory() as folder:
            ...
            lifecycle_path = (
                config.order_lifecycle_root / self.now.strftime("%Y%m%d")
            ...
            self.assertTrue(all(
                row["schema"] == "s02_order_lifecycle_v1"
                for row in lifecycle_rows
            ))
            self.assertEqual(len({row["signal_id"] for row in lifecycle_rows}), 1)
            self.assertEqual(
                len({row["idempotency_key"] for row in lifecycle_rows}), 1
            )
            self.assertTrue(all(
                set(row["production_files"]) == {
                    "engine", "shared_rotation_engine", "signal_contract",
                    "signal_source", "order_adapter"
                }
                for row in lifecycle_rows
            ))

            self.assertEqual(lifecycle_rows[-1]["signal_emitted_price"], 10_100)
            self.assertEqual(lifecycle_rows[-1]["arrival_price"], 10_100)
            self.assertEqual(lifecycle_rows[-1]["arrival_chase_bps"], 0.0)
            self.assertEqual(lifecycle_rows[-1]["fill_slippage_bps"], 0.0)
    def test_s02_loss_reentry_live_production_path(self) -> None:
        """Production-path replay: 15m + lower low + three confirmations."""
        with tempfile.TemporaryDirectory() as folder:
            root = Path(folder)
            now = self.now.replace(hour=10, minute=0, second=0, microsecond=0)
            exit_at = now - timedelta(minutes=16)
            base = build_config()
            config = replace(
                base,
                signal_path=root / "signal.json",
                ...
                lock_path=root / "engine.lock",
                live_requested=False,
                loss_reentry_gate_mode="LIVE",
                reentry_peer_state_paths=(root / "state.json",),
                reentry_audit_root=root / "audit",
            ...
            signal["updated_at"] = stamp
            signal["signals"][0].update({
                "ts": stamp,
                "code": "108490",
                "name": "ROBOTIS",
                "signal_sequence": 2,
            ...
            config.snapshot_path.write_text(json.dumps({"codes": {"108490": {
                "ts": stamp,
                "cur": 10_050,
                "cum_vol": 100_000,
                "buy_money_cum": 700_000_000,
            ...
            for offset in range(1, 16):
                bar_at = exit_at + timedelta(minutes=offset)
                low = 9_900 if offset == 5 else 9_950
                minutes.append(bar_at.strftime("%Y%m%d%H%M"))
                bars.append([10_000, 10_100, low, 10_050])
            config.bars_path.write_text(json.dumps({"m": {"108490": {
                "pm": minutes,
                "prev": bars,
            }}}), encoding="utf-8")
            config.names_path.write_text("{}", encoding="utf-8")
            ...
            engine = Strategy02Engine(
                ...
                broker=broker,
                slots=FakeSlots(),
                logger=logging.getLogger("s02-reentry-production-replay"),
                signal_selector=select_fresh_signals,
            )
            engine.state = engine._blank_state(now.strftime("%Y%m%d"))
            engine.state.update({
                "entered_codes": ["108490"],
                "cycles_by_code": {"108490": 1},
                "history": [{
                    "phase": "CLOSED",
                    "code": "108490",
                    "entry_price": 10_200,
            ...
            engine._try_entries(now)
            self.assertEqual(len(broker.submissions), 1)
            self.assertEqual(broker.submissions[0]["code"], "108490")

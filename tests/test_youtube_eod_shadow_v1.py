...
def test_eod_done_gate_requires_today_ok_quality_and_codes():
    payload = {
        "done_at": "2026-08-18T19:56:26",
        "status": "OK",
        "qa_score": 100.0,
        "codes": 1636,
    }
    assert shadow._eod_done_for_date(payload, "20260818") is True
    assert shadow._eod_done_for_date(payload, "20260819") is False
    assert shadow._eod_done_for_date({**payload, "status": "FAIL"}, "20260818") is False
    assert shadow._eod_done_for_date({**payload, "qa_score": 89.9}, "20260818") is False


def test_prepare_skips_broker_when_eod_done_gate_blocks():
    with (
        patch.object(shadow, "_now", return_value=datetime(2026, 8, 19, 16, 40)),
        patch.object(shadow, "_wait_for_eod_done", return_value=False),
        patch.object(shadow, "_broker_client") as broker,
    ):
        assert shadow.mode_prepare() == 2

...
class TrCallbackIsolationTests(unittest.TestCase):
    ...
    def test_late_response_does_not_exit_current_loop(self):
        ...
        self.assertEqual(0, gw._active_tr["loop"].exits)
    ...
    def test_matching_response_uses_its_frozen_fields(self):
        ...
        self.assertEqual(0, gw.tr_loop.exits)
        self.assertEqual(1, gw._active_tr["loop"].exits)
    ...
    def test_ordinary_tr_screens_rotate_inside_reserved_pool(self):
        gw = BrokerGateway.__new__(BrokerGateway)
        gw._tr_screen_seq = 0
        screens = [gw._next_tr_screen() for _ in range(200)]
        self.assertEqual(200, len(set(screens)))
        self.assertTrue(all(7000 <= int(screen) <= 7199 for screen in screens))
        self.assertEqual(screens[0], gw._next_tr_screen())
        self.assertEqual(
            "STRATEGY01_BALANCE",
            gw._wire_tr_rqname("STRATEGY01_BALANCE", stable=True),
        )

...
class BatchTrCapTests(unittest.TestCase):
    ...
    def test_batch_screen_release_is_scoped_and_wired_in_finally(self):
        """배치가 끝나는 모든 경로에서 사용 화면 하나만 해제해야 한다."""
        gw = self._cls().__new__(self._cls())

        class FakeOcx:
            def __init__(self):
                self.calls = []

            def dynamicCall(self, signature, screen):
                self.calls.append((signature, screen))

        gw.ocx = FakeOcx()
        gw._release_batch_tr_screen("2211")
        self.assertEqual(
            gw.ocx.calls,
            [("DisconnectRealData(QString)", "2211")],
        )

        source = (RUN_DIR / "broker_gateway_v1.py").read_text(encoding="utf-8")
        body = source.split("def _handle_batch_tr_request", 1)[1]
        finally_body = body.split("finally:", 1)[1].split("summary[", 1)[0]
        self.assertIn("self._release_batch_tr_screen(scr)", finally_body)

...
from captain2_common_foundation_v1 import ContractError, OrderLedger, OrderSide  # noqa: E402
...
class UnifiedHoldSellEngineTests(unittest.TestCase):
    ...
    def test_all_eight_strategy_profiles_exist(self):
        self.assertEqual(set(STRATEGY_PROFILES), set(StrategyId))
        self.assertEqual(len(CAPTAIN2_STRATEGIES), 8)
    ...
    def test_daily_ma_permit_doubles_final_flow_confirmation(self):
        state = self.state(StrategyId.REACCEL)
        poor = {
            "buy_ratio_recent": Decimal("0.40"),
            "structure_broken": True,
        ...
        self.engine.evaluate(state, self.observation(kst(10, 0), **poor))
        self.engine.evaluate(state, self.observation(kst(10, 0, 1), **poor))
        still_waiting = self.engine.evaluate(state, self.observation(kst(10, 0, 20), **poor))
        self.assertEqual(still_waiting.action, HoldSellAction.WATCH)
        sold = self.engine.evaluate(state, self.observation(kst(10, 0, 31), **poor))
        self.assertEqual(sold.action, HoldSellAction.SELL)

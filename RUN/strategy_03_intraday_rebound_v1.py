...
class IntradayReboundState:
    ...
    # ★[2026-08-13] 눌림저점 시각 — 계단 경로 수급 판정의 창 경계 앵커.
    pullback_low_ts: datetime | None = None
...
class IntradayReboundDetector:
    ...
    def _sell_decel_buy_flip_staircase(
        ...
        # ★[2026-08-13 친구님 지시 "계단 조건과 수급 기준시점 충돌만 최소 수정"]
        #   관문 의미(매도 감속 + 매수 가속 + 매수 우위)는 1레인과 동일하고,
        #   창 경계만 계단 앵커를 쓴다: 이전 구간 = 저점→눌림저점,
        #   최근 구간 = 눌림저점→현재. 고정 10초 격자는 계단 자료(성긴 틱)와
        #   어긋나 항상 fail-closed 였다(8/13 실측: nxt 정렬오차 6초 > 허용 4초).
        #   앵커·자료가 없으면 False = 안 산다(fail-closed 유지).
        if pullback_ts is None or low is None:
            return False
        mid = None
        for row in self.state.points:
            if row.ts <= pullback_ts:
                mid = row
            else:
                break
        if mid is None or mid.ts <= low.ts:
            return False
        prev_span = (mid.ts - low.ts).total_seconds()
        recent_span = (point.ts - mid.ts).total_seconds()
        if prev_span <= 0 or recent_span <= 0:
            return False
        prev_buy = max(0.0, mid.buy_money_cum - low.buy_money_cum) / prev_span
        prev_sell = max(0.0, mid.sell_money_cum - low.sell_money_cum) / prev_span
        recent_buy = max(0.0, point.buy_money_cum - mid.buy_money_cum) / recent_span
        recent_sell = max(0.0, point.sell_money_cum - mid.sell_money_cum) / recent_span
        return (
            recent_buy > prev_buy
    ...
    def feed(
        ...
        *,
        ...
        if not state.pullback_seen:
            ...
            return self._row(point, "WAIT", "PULLBACK_OR_SECOND_REBOUND_WAIT")
        if point.price < higher_low_floor:
            state.first_rebound_peak = 0.0
            state.pullback_seen = False
            state.pullback_low = 0.0
            state.pullback_low_ts = None
            return self._row(point, "WAIT", "SECOND_LOW_BROKE_FLOOR")
        ...
        if not self.config.min_rebound_pct <= rebound <= self.config.max_rebound_pct:
            return self._row(point, "WAIT", "RETEST_ENTRY_RANGE_MISS")

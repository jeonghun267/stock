...
def check_syntax() -> None:
    ...
    for p in sorted(RUN.glob("*.py")):
        try:
            # ★[NO-PYC 2026-08-10] py_compile 은 __pycache__ 에 .pyc 를 쓴다.
            #   RUN 을 읽기전용으로 잠그면(import 하이재킹 방어) 파일마다 쓰기가
            #   실패해 이 검사가 통째로 멎는다 — 8/10 아침에 실측했다(5분 넘게
            #   첫 항목에서 못 빠져나옴). 잠금을 넣으려면 여기가 먼저 바뀌어야 한다.
            #   compile() 에 bytes 를 주면 py_compile 과 똑같이 PEP 263 인코딩
            #   선언을 존중하면서 메모리에서만 컴파일한다(디스크 산출물 0).
...
def check_anchor_pipe() -> None:
    """★[공통배관 2026-08-07 친구님 "계약서에 안 바뀌게 못박아 놔"]

    시가·당일저가·당일고가 공통 배관(intraday_anchor_v1)의 '배선'을 잠근다.
    값이 아니라 모듈 존재·임포트·소스 우선순위·필드명·옛 경로 부재를 본다.
    """
    path = ROOT / "config" / "lowfind_contract_v1.json"
    try:
        c = json.loads(path.read_text(encoding="utf-8")).get("공통배관") or {}
    except (OSError, ValueError) as exc:
        add("FAIL", "공통배관 계약", f"계약서를 못 읽음 ({exc})")
        return
    if not c:
        add("FAIL", "공통배관 계약", "계약서에 공통배관 절이 없다")
        return
    bad = []

    module_path = ROOT / str(c.get("모듈파일") or "").replace("/", "\\")
    if not module_path.exists():
        bad.append(f"모듈 없음: {module_path.name}")
    else:
        try:
            import importlib
            mod = importlib.import_module("intraday_anchor_v1")
            if list(mod.SOURCE_ORDER) != list(c.get("소스우선순위") or []):
                bad.append(f"소스우선순위 {list(mod.SOURCE_ORDER)} != 계약 {c.get('소스우선순위')}")
            if list(mod.SNAPSHOT_FIELDS) != list(c.get("스냅샷필드") or []):
                bad.append(f"스냅샷필드 {list(mod.SNAPSHOT_FIELDS)} != 계약 {c.get('스냅샷필드')}")
        except Exception as exc:               # noqa: BLE001
            bad.append(f"모듈 import 실패: {exc}")

    # ★[AST 2026-08-07 보안검사 지적] 문자열 포함 검사에서 구문분석으로 바꿨다.
    #   종전 방식은 실제 배선 두 줄을 지우고 주석에만 anchor.open·anchor.low 를
    #   남겨도 PASS 였고, 반대로 주석에 옛 경로 이름을 적으면 멀쩡한 코드가 FAIL
    #   오탐이 났다. 같은 파일의 상승보유 계약 검사는 이미 "전부 ast 로 본다 —
    #   문자열 검색은 주석에 남은 이름까지 잡아 거짓 경보를 낸다"고 적어 뒀는데
    #   새 검사만 문자열로 후퇴했던 것이다. ast 는 주석을 아예 안 본다.
    def _tree(fname: str):
        try:
            return ast.parse((ROOT / "RUN" / fname).read_text(encoding="utf-8"))
        except (OSError, SyntaxError) as exc:
            bad.append(f"{fname}: 읽거나 해석할 수 없음 ({exc})")
            return None
    ...
    def _attr_reads(tree) -> set:
        """`anchor.open` 처럼 실제로 코드에 있는 속성 접근만 모은다(주석 제외)."""
        out = set()
        for node in ast.walk(tree):
            if isinstance(node, ast.Attribute) and isinstance(node.value, ast.Name):
                out.add(f"{node.value.id}.{node.attr}")
            # hr.get("first_price") 같은 옛 경로도 호출 형태로 잡는다
            if (isinstance(node, ast.Call)
                ...
                out.add(f'{node.func.value.id}.get("{node.args[0].value}")')
        return out

    for fname in c.get("모듈임포트필수") or []:
        tree = _tree(fname)
        if tree is not None and "intraday_anchor_v1" not in _imports(tree):
            bad.append(f"{fname}: 공통배관 모듈 임포트가 사라짐")
    for fname, needles in (c.get("배선문자열필수") or {}).items():
        tree = _tree(fname)
        if tree is None:
            continue
        reads = _attr_reads(tree)
        for needle in needles:
            if needle not in reads:
                bad.append(f"{fname}: 배선 '{needle}' 이 사라짐(코드에 없음)")
    for fname, needles in (c.get("금지문자열") or {}).items():
        tree = _tree(fname)
        if tree is None:
            continue
        reads = _attr_reads(tree)
        for needle in needles:
            if needle in reads:
                bad.append(f"{fname}: 옛 경로 '{needle}' 가 되살아남")

    # 계약서 자체가 비면 검사가 통째로 무의미해진다(계약서를 고치는 쪽이 더 쉽다).
    if not (c.get("모듈임포트필수") and c.get("배선문자열필수") and c.get("금지문자열")):
        bad.append("계약서의 검사 목록이 비었다 - 검사 범위가 사라졌다")

    if bad:
        add("FAIL", "공통배관 계약", f"{len(bad)}건 어긋남 - " + " | ".join(bad[:5]))
    else:
        add("PASS", "공통배관 계약",
            "모듈·임포트·우선순위·필드명·옛경로부재 전부 계약대로")
...
def check_tasks() -> None:
    ...
    required = [
        ...
        "STRATEGY06_LIVE", "SAFEPLUS_STRATEGY_ENGINE_RECOVERY",
    ]
    off = []
...
def check_s06_lifecycle() -> None:
    """S06이 최근 거래일에 실제로 기동·승인 갱신됐는지 산출물로 확인한다."""
    latest_session = _last_csv_date(DATA / "prices_1m.csv", col=1)
    if not latest_session:
        add("WARN", "S06 기동 이력", "1분봉 최신 거래일을 못 읽어 판정 보류")
        return

    bad = []
    approval = ROOT / "config" / "strategy_06_live_approved.flag"
    state = DATA / "strategy_06_crash_low_chase_state_v1.json"
    live_log = ROOT / "LOG" / "sched_STRATEGY06_LIVE.log"
    recovery_log = DATA / "LOG" / "sched_STRATEGY06_RECOVERY.log"

    try:
        token = approval.read_text(encoding="utf-8-sig", errors="replace")
        match = re.search(r"APPROVED_BY_OWNER\s+(\d{8})\s+S06_LIVE", token)
        if not match or match.group(1) < latest_session:
            bad.append("승인 갱신 없음")
    except OSError:
        bad.append("승인 파일 없음")
    ...
    try:
        ...
        if str(payload.get("date") or "") < latest_session:
            bad.append("상태 heartbeat 없음")
    except (OSError, UnicodeDecodeError, json.JSONDecodeError):
        bad.append("상태 파일 없음/손상")

    for label, path in (("실행 로그", live_log), ("복구 로그", recovery_log)):
        try:
            if path.stat().st_mtime < datetime.strptime(latest_session, "%Y%m%d").timestamp():
                bad.append(f"{label} 갱신 없음")
        except OSError:
            bad.append(f"{label} 없음")

    if bad:
        add("FAIL", "S06 기동 이력",
            f"최근 거래일 {latest_session}: " + " | ".join(bad))
    else:
        add("PASS", "S06 기동 이력",
            f"최근 거래일 {latest_session} 승인·상태·실행·복구 확인")


ELEVATED_BASELINE = ROOT / "config" / "elevated_path_hashes.json"

# ★[SEC-ELEVATED-HASH 2026-08-05 친구님 지시 "나머지 4개도 다 해줘"]
#   Highest(관리자 권한) 태스크가 실제로 실행하는 파일들. 2026-08-05 밤에
#   Get-ScheduledTask 로 뽑은 그대로다.
#
#   왜 지키나: UserK 는 Administrators 멤버지만 평소 프로세스는 Medium 이다
#   (UAC 토큰 필터 — 확인함). 그런데 이 파일들은 UserK 가 쓸 수 있다.
#   승격 안 된 프로세스가 한 개만 고쳐 두면 다음 실행 때 UAC 창 없이
#   관리자 권한으로 돈다. `_run_hidden_wait.vbs` 는 혼자서 태스크 8개의 관문이다.
#
#   ⚠️근본 차단은 태스크 RunLevel 을 Limited 로 낮추는 것이고 그건 관리자 셸에서만
#     된다(Medium 에서 Set-ScheduledTask 는 Access denied — 실제로 막혔다).
#     여기서는 '바뀌었는지'를 장 시작 전에 알려주는 데까지만 한다.
#   ⚠️전략 .py 는 일부러 뺐다 — 매일 밤 고치므로 넣으면 아침마다 FAIL 이 뜨고,
#     늘 뜨는 경보는 아무도 안 본다(그러면 진짜 변조를 오히려 가린다).
#     전략 .py 의 답은 탐지가 아니라 태스크 강등이다.
_ELEVATED_FILES = (
    r"RUN\_run_hidden_wait.vbs",
    r"RUN\broker_night_stop.ps1",
    r"RUN\run_safeplus_pipeline_0900.ps1",
    r"RUN\watchdog_broker_gateway_v1.ps1",
    r"RUN\hidden\SAFEPLUS_PREFLIGHT_SELFTEST_HIGH.cmd",
    r"RUN\hidden\SAFEPLUS_STRATEGY_ALL_PREFLIGHT.cmd",
    r"RUN\hidden\SAFEPLUS_STRATEGY_COMMON_CONTEXT.cmd",
    r"RUN\hidden\SAFEPLUS_STRATEGY01_LIVE.cmd",
    r"RUN\hidden\SAFEPLUS_STRATEGY01_SIGNAL.cmd",
    r"RUN\hidden\SAFEPLUS_STRATEGY02_LIVE.cmd",
    r"RUN\hidden\SAFEPLUS_STRATEGY02_SIGNAL.cmd",
    r"RUN\hidden\SAFEPLUS_STRATEGY03_LIVE_ASCII.cmd",
    r"RUN\hidden\SAFEPLUS_STRATEGY03_PREFLIGHT.cmd",
    r"RUN\hidden\SAFEPLUS_STRATEGY03_SIGNAL_ASCII.cmd",
    # 브로커 경로 — PIPELINE_0900 이 Highest 로 띄운다. 자체 모듈은 이 둘뿐이다.
    r"RUN\broker_gateway_v1.py",
    r"RUN\ipc_order_auth_v1.py",
)
...
def rebaseline_elevated() -> int:
    """지금 상태를 기준표로 굳힌다. 일부러 고친 뒤에만 쓸 것."""
    files, missing = {}, []
    for rel in _ELEVATED_FILES:
        path = ROOT / rel
        if path.exists():
            files[rel] = _sha256(path)
        else:
            missing.append(rel)
    ELEVATED_BASELINE.write_text(json.dumps({
        "_설명": "관리자 권한(Highest) 태스크가 실행하는 파일의 sha256 기준표.",
        "_왜": ("UserK 는 관리자 멤버지만 평소 프로세스는 Medium 이다. 이 파일들은 "
                "UserK 가 쓸 수 있고 Highest 로 실행되므로, 한 개만 고쳐 두면 "
                "UAC 창 없이 관리자 권한을 얻는다. 08:30 에 바뀐 게 있으면 알린다."),
        "_근본차단": ("태스크 RunLevel 을 Limited 로 낮추는 것. 관리자 셸에서만 된다. "
                      "브로커 계열 3개(PIPELINE_0900·WATCHDOG_BROKER·BROKER_NIGHT_STOP)는 "
                      "브로커를 죽이고 살려야 해서 Highest 가 필요하다."),
        "_다시만들기": (r"C:\python310\python.exe "
                        r"C:\stock_bot\RUN\morning_preflight_rehearsal_v1.py "
                        r"--rebaseline-elevated"),
        "_주의": "일부러 고친 뒤에만 다시 만들 것. 먼저 다시 만들고 나중에 확인하면 검사가 무의미해진다.",
        "생성": datetime.now().isoformat(timespec="seconds"),
        "파일": files,
    }, ensure_ascii=False, indent=2), encoding="utf-8")
    print(f"기준표 갱신: {len(files)}개")
    for rel in missing:
        print(f"  ⚠️없는 파일: {rel}")
    return 0


def check_elevated_path() -> None:
    try:
        saved = json.loads(ELEVATED_BASELINE.read_text(encoding="utf-8"))
        base = dict(saved.get("파일") or {})
    except Exception as exc:                   # noqa: BLE001
        add("FAIL", "관리자권한 실행파일", f"기준표를 못 읽었다 - {exc}")
        return
    changed, missing, unlisted = [], [], []
    for rel in _ELEVATED_FILES:
        path = ROOT / rel
        if not path.exists():
            missing.append(rel)
        ...
        want = base.get(rel)
        if want is None:
            unlisted.append(rel)               # 기준표에 없는 = 새로 늘어난 경로
        elif want != _sha256(path):
            changed.append(rel)
    bad = ([f"바뀜:{Path(r).name}" for r in changed]
           + [f"없음:{Path(r).name}" for r in missing]
           + [f"기준없음:{Path(r).name}" for r in unlisted])
    if bad:
        add("FAIL", "관리자권한 실행파일",
            f"{len(bad)}건 - " + " | ".join(bad[:6])
            + "  (일부러 고쳤으면 --rebaseline-elevated)")
    else:
        add("PASS", "관리자권한 실행파일",
            f"{len(_ELEVATED_FILES)}개 sha256 기준표와 동일")


# ══════════════════════════════════════════════════════════════
# ★[DATA-GUARD 2026-08-14 친구님 지시 "리허설 검사 항목 확대 해"]
#   왜 — 8/14 하루에 리허설이 못 잡은 고장이 셋 났다:
#     ① 일봉이 8/11 에서 3일 멈춰 있었는데 아무도 아침에 몰랐다
#        (고저폭판 후보 0건 → S06 대상 0 → 아침 내내 신호 0).
#     ② prices_1m.csv 가 8/11 이후 갱신 0 (백테·성과분석 기반 자료 결손).
#     ③ __pycache__ 에 8/10 자 낡은 .pyc 가 남아 그날 고친 코드가 안 떴다.
#        같은 수정을 세 번 "적용됐다"고 오판하게 만든 원인.
#   ⚠️셋 다 WARN 이다 — FAIL 은 FAIL_FLAG 를 써서 전략을 통째로 OFF 시킨다.
#     8/14 08:30 시점에 일봉은 아직 8/11 이었고 08:33 에 들어왔다. FAIL 로 넣었다면
#     그날 전략이 또 멈췄을 것이다. 먼저 며칠 관찰해 오탐이 없으면 그때 올린다.
#   되돌리기: morning_preflight_rehearsal_v1_20260814_before_data_checks.py
def _last_csv_date(path: Path, col: int = 0, tail_bytes: int = 65536) -> str:
    """큰 CSV 의 꼬리만 읽어 최신 날짜(YYYYMMDD)를 얻는다. 실패 시 빈 문자열."""
    try:
        ...
        for line in text.strip().splitlines()[1:]:
            ...
            if len(parts) <= col:
                continue
            v = parts[col].strip()[:8]
            if len(v) == 8 and v.isdigit() and v > best:
                best = v
        return best
    except OSError:
        return ""
...
def check_data_freshness() -> None:
    """일봉·1분봉이 낡지 않았는가. 낡으면 전략이 옛 자료로 판단한다."""
    today = datetime.now().strftime("%Y%m%d")
    eod = ROOT / "DATA" / "eod_daily_bars.csv"
    d = _last_csv_date(eod, col=0)
    if not d:
        add("WARN", "자료 신선도(일봉)", "eod_daily_bars.csv 최신일을 못 읽음")
    elif d < today:
        add("WARN", "자료 신선도(일봉)",
            f"최신일 {d} — 오늘({today}) 마감분 아직 없음. "
            f"08:40 고저폭판이 이 자료를 쓴다")
    else:
        add("PASS", "자료 신선도(일봉)", f"최신일 {d}")

    m1 = ROOT / "DATA" / "prices_1m.csv"
    d1 = _last_csv_date(m1, col=1)     # prices_1m 은 2번째 칸이 ts(YYYYMMDDHHMM)
    if not d1:
        add("WARN", "자료 신선도(1분봉)", "prices_1m.csv 최신일을 못 읽음")
    else:
        try:
            ...
            gap = 99
        # 문턱 2일 [친구님 지시 2026-08-14 "둘 다 해"] — 3일이면 8/14 당일(2일 결손)을
        # 놓친다. 하루 결손은 주말/휴일로도 생기므로 2일부터 경고한다.
        if gap >= 2:
            add("WARN", "자료 신선도(1분봉)",
                f"최신일 {d1} — {gap}일 결손. 성과분석·백테가 종가로만 계산된다")
        else:
            add("PASS", "자료 신선도(1분봉)", f"최신일 {d1}")


def check_stale_pyc() -> None:
    """소스보다 오래된 .pyc 가 있는가. 있으면 고친 코드가 안 뜬다."""
    cache = RUN / "__pycache__"
    if not cache.is_dir():
        add("PASS", "바이트코드 캐시", "__pycache__ 없음")
        return
    stale = []
    for pyc in cache.glob("*.cpython-*.pyc"):
        stem = pyc.name.split(".cpython-")[0]
        src = RUN / f"{stem}.py"
        try:
            if src.exists() and src.stat().st_mtime > pyc.stat().st_mtime + 1:
                stale.append(stem)
        except OSError:
            continue
    if stale:
        add("WARN", "바이트코드 캐시",
            f"소스보다 낡은 .pyc {len(stale)}개 — 고친 코드가 안 뜰 수 있다: "
            + ", ".join(sorted(stale)[:5]))
    else:
        add("PASS", "바이트코드 캐시", "낡은 .pyc 없음")


def main() -> int:
    if "--rebaseline-elevated" in sys.argv:
        return rebaseline_elevated()
    ...
    check_s06_lifecycle()
    check_stale_pyc()        # [DATA-GUARD 2026-08-14]

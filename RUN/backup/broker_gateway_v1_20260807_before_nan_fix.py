...
class BrokerGateway:
    ...
    def _handle_batch_tr_request(self, request_id, req):
        ...
        if len(codes) > self.BATCH_TR_MAX_CODES:
            ...
            return
        if batch_to > self.BATCH_TR_MAX_BATCH_SEC:
            logger.warning("[SEC-BATCHTR-CAP] batch_timeout %.1f -> %.1f (request_id=%s)",
                           batch_to, self.BATCH_TR_MAX_BATCH_SEC, request_id)
            batch_to = self.BATCH_TR_MAX_BATCH_SEC
        if per_request_to > self.BATCH_TR_MAX_PER_REQ_SEC:
            logger.warning("[SEC-BATCHTR-CAP] per_request_timeout %.1f -> %.1f (request_id=%s)",

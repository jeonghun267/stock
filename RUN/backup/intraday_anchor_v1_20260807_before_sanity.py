# -*- coding: utf-8 -*-
"""[공통배관 2026-08-07 친구님 지시 "시가 대비 저점을 실시간으로 알고 있어야 돼.
...
def day_anchor(
    ...
    *,
    ...
    def pick(hr_key: str, snap_key: str) -> tuple[float, str]:
        value = abs(_f(hr_row.get(hr_key)))
        if value > 0:
            return value, SOURCE_ORDER[0]
        value = abs(_f(snap_row.get(snap_key)))
        if value > 0:
            return value, SOURCE_ORDER[1]
        return 0.0, SOURCE_ORDER[2]

    open_px, src_o = pick(HR_FIELDS[0], SNAPSHOT_FIELDS[0])
    low_px, src_l = pick(HR_FIELDS[1], SNAPSHOT_FIELDS[1])
    high_px, src_h = pick(HR_FIELDS[2], SNAPSHOT_FIELDS[2])
    return DayAnchor(open=open_px, low=low_px, high=high_px,
                     source_open=src_o, source_low=src_l, source_high=src_h)

...
class BrokerGateway:
    ...
    def __init__(self):
        ...
        # Ordinary TRs use a broker-owned screen pool instead of reusing the
        # caller screen forever.  Kiwoom can keep replaying a stale callback on
        # a poisoned screen even when rqname is unique (2026-08-14 screen 0802).
        self._tr_screen_seq: int = 0
    ...
    def _on_receive_tr_data(self, screen_no, rqname, tr_code,
        ...
        try:
            ...
            request_loop = active.get("loop")
            if request_loop and request_loop.isRunning():
                request_loop.exit()
    ...
    def _next_tr_screen(self) -> str:
        """Return an isolated ordinary-TR screen from a broker-owned pool."""
        seq = (int(getattr(self, "_tr_screen_seq", 0)) + 1) % 200
    ...
    def _handle_sendorder_real_request(self, request_id, req):
        ...
        if order_type in _ORDER_CAP_BUY_SIDES:
            ...
            #   ② 그래도 가격을 못 구하면 수량과 무관하게 차단(fail-close).
    ...
    def _release_batch_tr_screen(self, screen_no):
        """BATCH_TR 전용 화면을 요청 종료 때 반드시 해제한다.

        타임아웃 난 TR 화면을 그대로 두면 Kiwoom 이 늦은 응답을 계속 보내 다음
        요청의 대기 루프를 방해한다. 이 함수는 해당 배치가 직접 사용한 화면 하나만
        해제하며 실시간 전략 화면 전체를 건드리지 않는다.
        """
        try:
            self.ocx.dynamicCall("DisconnectRealData(QString)", str(screen_no))
            logger.info("[BATCH_TR-SCREEN-RELEASE] screen=%s", screen_no)
        except Exception as exc:
            logger.warning(
                "[BATCH_TR-SCREEN-RELEASE] failed screen=%s error=%s",
                screen_no, exc,
            )
    ...
    def _handle_batch_tr_request(self, request_id, req):
        ...
        for i, code in enumerate(codes):
            ...
            try:
                ...
                _timer.timeout.connect(request_loop.quit)
                ...
                if active is not None and getattr(self, "_active_tr", None) is active:
                    self._active_tr = None
                # 성공·타임아웃·예외 어느 경로에서도 이 요청의 TR 화면을 남기지 않는다.
                # 8/18 screen 2211 지연 응답이 10초마다 반복된 재발을 차단한다.
                self._release_batch_tr_screen(scr)
    ...
    def _handle_tr_request(
        ...
        caller_screen_no = str(req.get("screen_no", "0001"))
        screen_no = (
            caller_screen_no
            if stable_wire_rqname
            else self._next_tr_screen()
        )
        ...
        try:
            ...
            timer.timeout.connect(request_loop.quit)
            ...
            active = {
                ...
                "caller_screen_no": caller_screen_no,
                "loop": request_loop,
            }
        ...
        try:
            ...
            for f in files:
                ...
                if self.state == BrokerState.RATE_LIMIT:
                    ...
                    continue
                # ★[POLL-SAFETY-PREEMPT 2026-08-19] 스냅샷(files)이 판 시작에 1회
                #   고정이라, 판 도중 도착한 잔고류 안전조회(우선순위 1)가 남은
                #   일반 TR(5) 꼬리에 묶여 클라이언트 예산(매도 전 1.0s)을 넘겼다
                #   — 8/19 BALANCE-RETRY 8회·매도 확인 지연의 남은 원인. BATCH_TR
                #   슬라이스 선점(:3808)과 같은 패턴을 일반 poll 에도 둔다.
                #   주문(0)은 50ms 긴급 타이머가 중첩 대기 중에도 이미 처리한다.
                if _ipc_request_sort_key(f)[0] > 1:
                    for safety_path in _pending_safety_query_paths():
                        if safety_path.exists():
                            self.process_request(safety_path)
                    if not f.exists():
                        continue

# -*- coding: utf-8 -*-
# ★[SHADOW-C 2026-08-05] 그림자 전용 사본 — 주문 없음, 실전과 무관.
#   낙폭 구간별 반등 문턱 사다리만 원본과 다르다. 나머지는 동일.
...
# ★[S03-ALIGN 2026-08-05 친구님 지시] 저점 찾는 법을 S03 와 같은 값으로 맞춘다.
#   S02 의 6번식(SIX_*)은 S03·S06 과 코드 구조가 완전히 같다(low -> dead_low ->
#   first_rebound_peak -> pullback_low -> 재무장, 필드명까지 동일). 그런데 8/3 에
#   S03 만 개편되고 S02 는 옛 값으로 남아 있었다 — 복사본이 3개라 한쪽만 고쳐진 것.
#   9개 값 중 6개는 이미 같았고 어긋난 건 아래 둘 + 관찰시간뿐이었다.
#     1차 반등 문턱  1.5 -> 1.0  (S03 와 동일)
#     추격 상한     2.0 -> 1.5  (S03 와 동일)
#   ⚠️관찰시간(SIX_OBSERVE_SEC=60)은 바꾸지 않는다 — 친구님: "2번은 저점에서
#     60초 관찰이 있고, 3번은 급반등이라 관찰이 없다". 전략 정체성이라 통일 대상이 아님.
#   실증: 8/5 원익IPS(240810) 를 저점(96,000 @10:26) 대비 +1.875% 에서 매수했다.
#     추격 상한이 2.0 이라 통과했지만 S03 값(1.5)이었으면 그 자리는 차단됐다.
#     10:40 에 +0.73%(거래량 1,048, 조용한 다지기) 자리가 있었는데 1차 반등 문턱
#     1.5% 를 먼저 봐야 해서 구조적으로 못 샀다(1.5% 도달은 10:50).
#   되돌리기: backup\strategy_02_low_buy_signal_v1_20260805_before_s03_align.py
SIX_FIRST_REBOUND_PCT = float(os.environ.get("S02_SIX_FIRST_REBOUND_PCT", "1.0"))
SIX_CHASE_CAP_PCT = float(os.environ.get("S02_SIX_CHASE_CAP_PCT", "1.5"))

# ★[SHADOW-C 2026-08-05] 낙폭 구간별 반등 문턱 사다리 (친구님 가설).
#   근거: 일봉 1년 144,334 종목일 실측 — 낙폭이 깊을수록 반등 여력이 크다.
#     낙폭 -3~-4%  저점→당일고가 중앙 4.69%
#          -4~-6%                    6.22%
#          -6~-8%                    8.45%
#          -8~-10%                  10.95%
#          -10~-12%                 13.24%
#          -12~-15%                 16.72%
#          -15%이상                 24.47%
#   문제: 고정 문턱 1.0% 는 얕은 낙폭에선 반등의 21% 를 먹고(늦음),
#         깊은 낙폭에선 4% 밖에 안 된다(여유 남음).
#   사다리는 '반등 여력의 약 10%' 를 문턱으로 잡는다.
#   하한 = 반등문턱, 상한 = 반등문턱 × 1.5 (현행 구조 비율 유지).
_SHADOWC_LADDER = (
    (4.0, 0.50), (6.0, 0.60), (8.0, 0.85), (10.0, 1.10),
    (12.0, 1.30), (15.0, 1.70), (999.0, 2.40),
)


def _shadowc_rebound_pct(drop_pct: float) -> float:
    for edge, value in _SHADOWC_LADDER:
        if drop_pct < edge:
            return value
    return _SHADOWC_LADDER[-1][1]
...
def _s01_taken_codes(today: str) -> set:
    ...
    진입창을 09:00 로 당기면서 1번·3번과 시간이 겹치게
...
class LowBuySignalMonitor:
    ...
    def _detect_six_staircase(
        ...
        # [SHADOW-C] 낙폭 구간별 사다리로 문턱을 고른다
        _sc_drop = (
            (state.six_episode_high - state.six_low)
            / state.six_episode_high * 100.0
            if state.six_episode_high > 0 and state.six_low > 0 else 0.0)
        _sc_reb = _shadowc_rebound_pct(_sc_drop)
        rebound_floor = state.six_low * (1.0 + _sc_reb / 100.0)
        chase_ceiling = state.six_low * (1.0 + _sc_reb * 1.5 / 100.0)
        entry_floor = state.six_low * (1.0 + _sc_reb / 100.0)

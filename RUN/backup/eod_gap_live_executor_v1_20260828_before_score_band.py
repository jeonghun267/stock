...
def _entry_config_snapshot():
    return {
        ...
        "MIN_SCORE": MIN_SCORE, "DR_CEIL": DR_CEIL, "VR_MIN": VR_MIN,
...
def _start_entry_audit(today, top, cands, positions):
...
def _replay_entry_audit(audit_path):
    ...
    try:
        ...
        if capture_hash != replay_hash:
            raise ValueError("capture/replay engine hash mismatch")
        ...
        if eligible < 1:
            raise ValueError("no complete order-eligible entry path")
...
def _buy_one(bc, pick, today, held, setup_route=False):
    ...
    # 전략 A/B 자체 패턴이 진입 게이트다. 일반 점수 하한을 다시 적용하지 않는다.
...
def mode_pick():
    ...
    _start_entry_audit(today, top, cands, _existing)

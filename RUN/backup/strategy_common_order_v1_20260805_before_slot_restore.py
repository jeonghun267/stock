...
class StrategyBroker:
    ...
    def _balance_tr_retry(self, **kwargs) -> Dict[str, Any]:
        """status != OK 면 짧게 쉬고 다시 묻는다. 마지막 응답을 그대로 돌려준다."""
        response: Dict[str, Any] = {}
        for attempt in range(1, self._BALANCE_RETRY + 1):
            ...
            if str(response.get("status") or "").upper() == "OK":
                ...
                return response
            if attempt < self._BALANCE_RETRY:
                _time.sleep(self._BALANCE_RETRY_SLEEP)
        self.log.warning(
            "[BALANCE-RETRY] %s %d회 모두 실패: %s",
            kwargs.get("tr_code", "?"), self._BALANCE_RETRY,

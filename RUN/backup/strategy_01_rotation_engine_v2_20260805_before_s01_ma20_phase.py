...
class Config:
    ...
    quantity: int = int(os.environ.get("S01_QTY", "2"))
    ...
    def __post_init__(self) -> None:
        if self.quantity not in {1, 2}:
            raise ValueError(f"{self.strategy_label} quantity must be one or two shares")
        if self.max_slots != 6:
...
class Strategy01Engine:
    ...
    def _daily_ma_permit(
        ...
        """
        # ★[MA20-ALLOW 2026-08-05 친구님 지시 "지금 켜"] S01 만 20선 단계를 다시
        #   상승보유에 넣는다. 8/4 에 공용으로 막았던 것(allow_ma20 기본 False)을
        #   1번에 한해 되돌린다 — 트레일을 8/4 에 껐기 때문에 이제 상승보유가
        #   막아서는 대상이 트레일이 아니라 꼭지수급·수급역전이다.
        #   되돌리기: 이 호출과 아래 _build_observation 의 allow_ma20 인자를 빼면 끝.
        #   백업: backup\strategy_01_rotation_engine_v2_20260805_before_ma20_allow.py
    ...
    def _build_observation(
        ...
        ma_hold = ma3_rider_permit(
            ...
            allow_ma20=True,   # ★[MA20-ALLOW 2026-08-05] 위 _daily_ma_permit 주석 참조
        )
        return HoldSellObservation(
            ...
            #   S01 은 위 ma_hold 가 이미 allow_ma20=True 로 구해진 값이라 그대로
            #   쓴다 = 같은 값 = S01 동작은 변하지 않는다. 뜻이 있는 건 S02·S05 다
            #   (그쪽은 daily_ma_permit 을 allow_ma20 없이 따로 구해 덮어쓴다).
            ma20_defense_permit=ma_hold,

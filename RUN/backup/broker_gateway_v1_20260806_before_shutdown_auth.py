...
from ipc_order_auth_v1 import verify_order_request
...
class BrokerGateway:
    ...
    def _ipc_auth_ok(self, request_id, req, expected_type, tag) -> bool:
        ...
        ★[IPC-AUTH-BLANKET 2026-08-05] 잠글 분기가 넷이 되면서 같은 12줄을 네 번
          쓰게 돼 한 곳으로 모았다. 기존 두 분기(SENDORDER_REAL·SET_REAL_REMOVE_ALL)
          는 일부러 손대지 않는다 — 실주문 경로를 이번 변경에 끌어들이지 않는다.
        """
    ...
    def process_request(self, req_path: Path):
        ...
        try:
            ...
            if req_type == "PING":
                ...
                auth_ok, auth_error = verify_order_request(
                    req, expected_type="SET_REAL_REMOVE_ALL")
                if not auth_ok:
                    logger.error(
                        "[SEC-REALREMOVE-AUTH] rejected request_id=%s caller=%s reason=%s",
                        request_id, str(req.get("caller") or "")[:40], auth_error,
                    )
                    self._write_response(
                        request_id,
                        status="ERROR",
                        error=(
                            "SET_REAL_REMOVE_ALL authentication rejected: "
                            f"{auth_error}"
                        ),
                    )
                else:
                ...
                auth_ok, auth_error = verify_order_request(
                    req, expected_type="SENDORDER_REAL")
                if not auth_ok:
                    logger.error(
                        "[SEC-ORDER-AUTH] rejected request_id=%s caller=%s reason=%s",
                        request_id, str(req.get("caller") or "")[:40], auth_error,
                    )
                    self._write_response(
                        request_id,
                        status="ERROR",
                        error=f"SENDORDER_REAL authentication rejected: {auth_error}",
                    )
                else:

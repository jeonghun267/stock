...
def three_minute_closes(code: str, payload: Dict[str, Any]) -> List[float]:
    ...
    for index, bar in enumerate(previous):
        ...
        if use_labels:
            ...
            if len(tag) == 12 and tag.isdigit():
                ...
                minute = current_day.toordinal() * 1440 + int(tag[:2]) * 60 + int(tag[2:])
            else:
                continue
    ...
    for block in sorted(groups):
        if block == current_absolute // 3:    # 진행 중 블록 제외
            continue
...
def line_support(code: str, price: float,
    ...
    if not row or price <= 0:
        return False
    if price >= row["ma10"]:                       # 10선이 받치면 보유
        return True
    return bool(                                    # 10선 침범 시 20선 우상향·지지
        row["ma20"] > row["ma20_prev"]
        and price >= row["ma20"])
...
def rider_permit(code: str, price: float,
    ...
    """Common MA3 rider: MA10 support or rising-MA20 support.

    buy_side is retained for caller compatibility but is not a hold gate.
    """

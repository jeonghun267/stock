...
def _moneyflow_codes(now: dt.datetime) -> set[str]:
    """오늘 브로커 장부에 mflow 주문 키가 찍힌 종목 = 돈맥 실행기 소유 근거.

    ★[MFLOW-LABEL 2026-08-19 친구님 지시 "공통 기존이 돈맥이면 돈맥으로 바꿔줘"]
    판별 근거는 LOG\\broker_journal.log 의 SENDORDER-REAL key 접두어(8/19 12종목
    전건 판별 검증). 읽기 전용 — 실패하면 빈 집합(라벨만 종전 공통/기존으로 남는다).
    """
    ...
    for path in sorted(LOG_DIR.glob("broker_journal.log*")):
        try:
            if dt.datetime.fromtimestamp(path.stat().st_mtime).date() < now.date():
                continue  # 회전된 지난날 장부는 건너뛴다(오늘 체결만 필요)
...
def load_strategy_events(now: dt.datetime) -> tuple[list[dict[str, str]], dict[str, str]]:
    ...
    # 전략 1~6 로그에서 주인을 못 찾은 체결 중, 브로커 장부의 mflow 주문 키로
    # 확인된 종목만 [돈맥]으로 표시. 나머지는 종전대로 공통/기존.
    try:
        for code in _moneyflow_codes(now):
            source_by_code.setdefault(code, "돈맥")
    except Exception:
        pass
...
def render_html(snapshot: dict[str, Any]) -> str:
    ...
    parts.append(
        ...
        f'<td>{money_buys}</td><td>{len(money_rows)}/{money_univ}</td>'
        '<td>-</td><td>-</td></tr>'
    )
    parts.append("</table>")

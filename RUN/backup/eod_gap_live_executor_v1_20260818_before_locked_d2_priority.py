...
def _pick_window_open(now=None):
    """Allow new EOD_GAP entries only during 15:00:00-15:25:59."""
    now = now or datetime.now()
    return (15, 0) <= (now.hour, now.minute) < (15, 26)
...
def _auction_snapshot(bc, code):
    """키움 공식 종가 동시호가 FID(23 예상체결가·24 예상체결수량)를 보존한다."""
    code = str(code).zfill(6)
    reg = bc.setreal_reg(AUCTION_SCREEN, code, AUCTION_FIDS,
                         real_type="1", timeout_sec=1.5)
    raw = {str(fid): _real_value(bc, code, fid) for fid in (21, 23, 24, 121, 125)}
    record = {
...
def _auction_gate_decision(record, now=None):
    ...
    return True, f"px={expected_px:.0f} qty={expected_qty:.0f} imb={imbalance:.3f} age={age:.0f}s"
...
def _buy_one(bc, pick, today, held):
    ...
    if not order_ok:
        ...
        if LIVE:
            return False
    held[code] = {"code": code, "name": nm, "qty": qty, "buy_price": px, "score": sc,
                  "date": today, "status": "OPEN", "live": LIVE, "ts": datetime.now().isoformat()}
    if LIVE:
        rt = _jload(RT_OPEN)

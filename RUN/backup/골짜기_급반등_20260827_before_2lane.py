...
class RapidReboundConfig:
    ...
    def __post_init__(self) -> None:
        # ★[2026-08-27] 인계 하한 폐지 — 계단 무장선(-4%)만 음수 검증한다.
        if not self.arm_drop_pct < 0:
            raise ValueError("arm drop must be negative")
...
class RapidReboundDetector:
    ...
    def feed(self, point: MicroPoint, profile: PriorProfile, *, allow_signal: bool) -> dict[str, Any]:
        ...
        #   [2026-08-27 친구님 지시 "6번에 공급하는 것은 있으면 안 돼"] 인계 잔재 완전 제거.
        ...
        if not state.armed:
            # ★[2026-08-27 친구님 지시 "express_deep 무장 조건 유지해"] 급행 매수는 폐지됐지만
            #   깊은 폭락 무장은 남긴다 — 시가 위에서 급등 후 폭락한 종목(7/29 376900:
            #   시가 28,000 → 33,500 → -15%)도 당일 고점 -7%↓면 저점 추적을 시작한다.
            #   급행 폐지 + 깊은구역 차단 제거로, 이런 종목도 이제 계단(4단계)이 직접 잡는다.
            deep_arm = (
                state.day_high > 0
                and (point.price / state.day_high - 1.0) * 100.0 <= EXPRESS_DEPTH_PCT
            )
            if point.price > arm_price and not deep_arm:
                state.last = point
...
def run(config: SignalConfig, *, once: bool = False) -> int:
    ...
    while True:
        ...
        combined_watch = {"for_date": day, "codes": sorted(early_codes)}

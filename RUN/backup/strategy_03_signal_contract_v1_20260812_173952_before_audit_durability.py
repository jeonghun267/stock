...
class EarlyLowAuditChain:
    ...
    def append(self, record: Mapping[str, Any]) -> dict[str, Any] | None:
        ...
        # ★[2026-08-12 보안수리 3번] 이 박스 고질병인 순간 파일잠금(WinError 5)에
        #   감사 쓰기가 걸려 '증거 없는 주문'이 생기지 않도록, 최초 1회 + 0.1초 간격
        #   3회 재시도로 감싼다. append 모드 open 이 실패하면 write 자체가 없으므로
        #   부분 줄이 남지 않는다. 끝까지 실패하면 None 을 돌려 호출부(selector)가
        #   그 신호의 실주문을 보류하게 한다(주문은 이 실패를 무시하지 않는다).

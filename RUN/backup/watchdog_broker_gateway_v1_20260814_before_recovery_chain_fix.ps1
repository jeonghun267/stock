...
# 08-14 16:05 the evening collector jammed for 2h29m: 98 TIMEOUT, 0 saved, file mtime frozen
# at 08:33, while heartbeat/PID kept looking healthy.  The TR-JAM detector above could not
# see it for two independent reasons, so it is left untouched here:
#   1. its window ends at 15:30, and
#   2. it reads broker_journal.log, which mixes every client - other clients kept receiving
#      OK responses all through the jam (17:00 window: OK=6, TIMEOUT=33), so the OK-famine
#      test can never hold even if the window were widened.
# This watcher therefore judges the collector by its OWN two artifacts, and demands both:
#   A. heartbeat is fresh but its idx counter has not moved for EOD_IDX_STALL_SEC, and
#   B. over that same span the collector log shows zero successes and only timeouts.
# Either signal alone is ambiguous (idx also pauses during a slow save; timeouts also occur
# in healthy runs), so only their conjunction is called a zombie.
# Fail-closed on doubt: any unreadable/short/empty evidence means NO kill.
...
$EOD_CONNECT_WAIT_SEC = 90    # broker CONNECTED wait before the collector is restarted
...
# [EOD-COLLECTOR-ZOMBIE 2026-08-14] Evidence B - read the collector's own log over one span.
# Returns ok / timeouts / verdict, where verdict is one of:
#   "zombie"       zero successes and enough timeouts to be sure
#   "healthy"      at least one success line in the span
#   "insufficient" log missing, unreadable, or too few lines to judge
# A success is either an explicit status=OK or a progress line (the collector prints one per
# 100 codes; its ASCII tail "ETA=" is matched so this file stays ASCII-only).
...
# [EOD-COLLECTOR-ZOMBIE 2026-08-14] Evidence A - heartbeat fresh but idx frozen.
# The stall clock lives in script scope and only advances across real watchdog cycles, so a
# freshly started watchdog cannot declare a stall it never observed.
$script:EodLastIdx   = -1
$script:EodIdxMovedAt = [datetime]::MinValue
function Is-Eod-Collector-Zombie() {
    try {
        if (-not (Test-Path $EOD_HB_FILE)) { return $false }
        $hb = Get-Content $EOD_HB_FILE -Raw -ErrorAction Stop | ConvertFrom-Json
        $now = Get-Date
        ...
        if ($script:EodIdxMovedAt -eq [datetime]::MinValue) {   # first sighting, nothing proven
            $script:EodIdxMovedAt = $now
            return $false
        }
        ...
        Write-Log "EOD collector check unreadable ($_) - treated as normal, no action"
        return $false
    }
}

# [EOD-COLLECTOR-ZOMBIE 2026-08-14] Recovery order is fixed: stop the collector first so it
# cannot fire fresh TRs into a broker that is about to die, then restart the broker and wait
# for CONNECTED, then bring the collector back.  Any failed step aborts the rest - a collector
# started against a dead broker would simply reproduce the jam.
function Restart-Eod-Collector-Chain() {
    try {
        $hb = Get-Content $EOD_HB_FILE -Raw -ErrorAction Stop | ConvertFrom-Json
        $cpid = [int]$hb.pid
    } catch {
        Write-Log "  EOD recovery aborted: heartbeat unreadable"
        return $false
    }
    Write-Log "EOD recovery step 1: stop collector PID=$cpid"
    try {
        Stop-Process -Id $cpid -Force -ErrorAction Stop
        Write-Log "  collector PID=$cpid stopped"
    } catch {
        Write-Log "  collector stop failed: $_ - recovery aborted"
        return $false
    }
    Write-Log "EOD recovery step 2: restart broker"
    if (-not (Restart-Broker)) {
        Write-Log "  broker restart failed - collector NOT restarted (would jam again)"
        return $false
    }
    Write-Log "EOD recovery step 3: wait for broker CONNECTED"
    $deadline = (Get-Date).AddSeconds($EOD_CONNECT_WAIT_SEC)
    $connected = $false
    while ((Get-Date) -lt $deadline) {
        Start-Sleep -Seconds 5
        try {
            $bhb = Get-Content $HB_FILE -Raw -Encoding UTF8 -ErrorAction Stop | ConvertFrom-Json
            $bage = ((Get-Date) - (Get-Item $HB_FILE).LastWriteTime).TotalSeconds
            $bpid = [int]$bhb.pid
            if ($bage -lt 15 -and $bpid -gt 0 -and (Is-Target-Pid-Alive $bpid)) {
                $connected = $true
                Write-Log "  broker CONNECTED (PID=$bpid heartbeat age=$([int]$bage)s)"
                break
            }
        } catch {}
    }
    if (-not $connected) {
        Write-Log "  broker CONNECTED not confirmed in ${EOD_CONNECT_WAIT_SEC}s - collector NOT restarted"
        return $false
    }
    Write-Log "EOD recovery step 4: restart collector task $EOD_TASK_NAME"
    try {
...
while ($true) {
    ...
    if ($hb_alive) {
        ...
        # [EOD-COLLECTOR-ZOMBIE 2026-08-14] Runs on the same 60s spacing and before the broker
        # checks below: when the collector is the stuck party, its own recovery chain restarts
        # the broker in the correct order, so letting the broker path fire first would leave a

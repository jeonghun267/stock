...
def make_strategy03_signal_selector(
    ...
    _audit_seen: set[tuple[str, str, bool, bool]] = set()
    ...
    def _audit_early_rows(payload, rows, validated, snapshot, now, max_age_sec,
        ...
        for row in early_rows:
            ...
            key = (code, str(row.get("ts") or ""))
            contract_pass = key in contract_keys
            selector_pass = key in selected_keys
            seen_key = (code, str(row.get("ts") or ""),
                        contract_pass, selector_pass)
            if seen_key in _audit_seen:
                continue
            _audit_seen.add(seen_key)
            raw = codes.get(code)
            chain.append({
                "event": "ORDER_GATE",
                "code": code,
    ...
    def selector(
        ...
        *,
        ...
        except Exception as exc:  # noqa: BLE001 — 감사 실패가 주문을 막으면 안 된다
            print(

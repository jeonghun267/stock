...
class Strategy01Engine:
    ...
    def _try_entries(self, now: datetime) -> None:
        if self.state.get("recovery_blocked"):
            return
        # S01 starts before the all-strategy preflight so opening signals are
        # not lost to process startup.  Do not even select/consume a signal
        # until the existing live gate has actually enabled buys.  Subclasses
        # used by S02/S03 keep their original entry path unchanged.
        if self.config.strategy_id == StrategyId.S01_OPEN_SURGE:
            ...
            if not buy_gate_allowed:
                if self._last_s01_buy_gate_allowed is not False:
                    self.log.critical(
                        "S01_BUY_GATE_CLOSED standby; no signal consumed"
                    )
                    self._event(
                        "BUY_GATE_CLOSED",
            ...
            if self._last_s01_buy_gate_allowed is False:
                self.log.info("S01_BUY_GATE_OPEN entries enabled")

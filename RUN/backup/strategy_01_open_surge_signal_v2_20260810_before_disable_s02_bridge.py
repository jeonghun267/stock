...
#
#   ★기본값 = 켜짐. 친구님 결론이 "1번 매수방법이 잘못됐고 2번이 맞다" 이므로
#     이것이 이제 1번의 정상 동작이다. 환경변수는 되돌리기 스위치로만 쓴다.
#     (기본값을 켜짐으로 둔 이유: 예약 태스크가 환경변수를 못 받아도 확실히 적용되게)
#     되돌리기: setx S01_USE_S02_LOWFIND NO   (프로세스 재기동 후 적용)
USE_S02_LOWFIND = (
    os.environ.get("S01_USE_S02_LOWFIND", "YES").strip().upper()
    in {"YES", "Y", "1", "TRUE", "ON"}
)

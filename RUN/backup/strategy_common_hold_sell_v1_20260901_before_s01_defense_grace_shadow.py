...
STRATEGY_PROFILES = {
    **STRATEGY_PROFILES,
    ...
    StrategyId.S02_LOW_BUY_SELL_EXHAUSTION: _profile(
        ...
        # Owner-approved permanent S02 profit protection (2026-08-14).
...
class HoldSellConfig:
    ...
    # Capture/validation first. Do not activate this order-capable hold rule
    # until a complete production replay passes.
    ...
    #       15분 +0.64%(11/20) · 30분 +0.73%(12/19) · 60분 +0.31%(13/21)
    #     즉 현행 꼭지매도가 조금 이르다. 8/14 하나마이크론이 대표 사례:
    #       꼭지 +3.12% 에서 -0.96% 밀리자 매도했는데 그 시점에
    #       price>MA5·MA5상승·MA10지지·MA20지지가 전부 살아 있었다(15분 뒤 +1.53% 더 감).
    #   ⚠️표본 22건은 적다. 편차도 크다(+6.05% ~ -3.41%). 그림자 2주를 먼저 권했으나
    #     친구님이 "그림자는 관리를 못 한다"고 실전을 택했다(오늘 종가매수 그림자가
    #     3일 멈춘 것을 아무도 몰랐던 전례가 있어 타당한 판단).
    #   ⚠️조건에 return_pct>0 이 있어 **손실 구간에서는 절대 발동하지 않는다** —
    #     하드손절·시간청산은 이 보류보다 앞에서 처리되므로 손실을 늘리지 않는다.
    #   롤백: setx S02_MA5_TREND_HOLD_DATE ""  (또는 이 줄 삭제)
    s02_ma5_trend_hold_active_date: Optional[str] = (
        os.environ.get("S02_MA5_TREND_HOLD_DATE") or None
    )
...
class HoldSellObservation:
    ...
    ma5_value: Decimal = Decimal("0")
    ma5_prev_value: Decimal = Decimal("0")
    ma10_value: Decimal = Decimal("0")
    ma3_source: str = ""
...
class HoldSellState:
    ...
    entry_lane: str = ""
    # S03 09:20 이전 골짜기의 MA5 추세 인계 상태.
    s03_open_ma5_handoff: bool = False
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "s03_open_ma5_handoff": self.s03_open_ma5_handoff,
        }
    ...
    def from_dict(cls, payload: Mapping[str, Any]) -> "HoldSellState":
        ...
        state.s03_open_ma5_handoff = bool(
            payload.get("s03_open_ma5_handoff", False)
        )
        return state
...
class UnifiedHoldSellEngine:
    ...
    def _common_peak_flow_exit_rule(
        ...
        # ★[S02-MA-HOLD-POS 2026-08-14 친구님 지시 "판단 위치만 최소 수정"]
        #   왜 여기인가 — rules_rider_first 순서상 이 꼭지매도가 _daily_ma_hold_rule
        #   보다 앞이라, MA 보류를 뒤쪽 규칙에 넣으면 **도달조차 못 한다**.
        #   8/14 하나마이크론이 그랬다: 꼭지 +3.12% 에서 -0.96% 밀려 여기서 팔렸고,
        #   그 시점 price>MA5·MA5상승·MA10지지·MA20지지가 전부 살아 있었다.
        #   그래서 조건이 서면 이 규칙을 '판정하지 않고' 넘긴다(None) — 뒤의
        #   _daily_ma_hold_rule 이 HOLD 를 낸다.
        #   ⚠️수급 준비 여부(common_peak_flow_ready)보다 **앞**에 둔다 — 지시대로
        #     수급 데이터가 준비되지 않아도 동일하게 발동해야 한다.
        #   ⚠️하드손절·시간청산은 이 규칙보다 앞이라 그대로 먼저 처리된다(무변경).
        #   ⚠️S01 및 다른 전략은 이 분기에 들어오지 않는다(strategy_id 검사).
        #   롤백: setx S02_MA5_TREND_HOLD_DATE ""
        if (
            self.config.s02_ma5_trend_hold_active_date is not None
            and (
        ...
        supported_weak_reversal = bool(
            ...
            and not observation.daily_ma_permit
        )
        if supported_weak_reversal:
            arm_token = format(
                profile.supported_weak_peak_arm_return_pct, "f"
            ).rstrip("0").rstrip(".").replace(".", "P")
            drop_token = format(
                profile.supported_weak_peak_drop_pct, "f"
            ).rstrip("0").rstrip(".").replace(".", "P")
            rule_name = (
                f"S02_PEAK_{arm_token}_DROP_{drop_token}_FLOW_"
                f"{profile.supported_weak_peak_score}OF4"
            )
        ...
        s01_ma5_trend_hold = bool(
            self.config.s01_ma5_trend_hold_active_date is not None
            and (
                self.config.s01_ma5_trend_hold_active_date == "*"
                or observation.observed_at.strftime("%Y%m%d")
                == self.config.s01_ma5_trend_hold_active_date
            )
            and state.strategy_id is StrategyId.S01_OPEN_SURGE
            and state.quantity == 1
            and return_pct > 0
            and observation.price_above_ma5
            and observation.ma5_rising
        ...
        # [2026-08-14 계약 복원] 8/13 배선이 daily_ma_permit 에 or 를 붙여
        # 리허설 계약(상승보유.매수세관문우회금지: 조건식은 daily_ma_permit 단독)을
        # 위반 → 8/14 08:30 FAIL → S01~S03 자동 OFF. S01 MA5 보류(친구님 8/13
        # 상시 ON 지시)는 별도 가지로 분리 — 동작 동일, COMMON_RISING_HOLD 는
        # 다시 daily_ma_permit 하나로만 나간다.
        # ★[S02-MA-HOLD 2026-08-14 친구님 지시] S02 도 같은 MA5 추세보류.
        #   S01 가지와 별개 변수로 둔다 — 날짜 스위치가 전략별로 독립이어야
        #   한쪽만 끄고 켤 수 있다(S01 은 8/17 첫 실전, S02 는 실측 22건 근거).
        #   조건은 S01 과 동일: 1주 · 수익>0 · price>MA5 · MA5상승 · MA10지지.
        s02_ma5_trend_hold = bool(
            self.config.s02_ma5_trend_hold_active_date is not None
            and (
                self.config.s02_ma5_trend_hold_active_date == "*"
                or observation.observed_at.strftime("%Y%m%d")
    ...
    def _evaluate_trend_rebound(
        ...
        s03_open_handoff_gate = bool(
            state.strategy_id is StrategyId.VALLEY_MORNING_CRASH
            and profile.exit_policy is ExitPolicy.TREND_REBOUND
        )
        if (
            s03_open_handoff_gate
            and not state.s03_open_ma5_handoff
            and observation.price_above_ma5
            and observation.ma5_rising
        ):
            state.s03_open_ma5_handoff = True
        trend_handed_off = bool(
            not s03_open_handoff_gate or state.s03_open_ma5_handoff
        )
        if trend_handed_off:
        ...
        return self._decision(
            ...
            (
                "TREND_REBOUND_HOLD"
                if trend_handed_off
                else "S03_OPEN_PRE_MA5_HANDOFF_HOLD"
            ),
        )

...
# ★[MA3-RIDER 2026-08-19 친구님 지시 "그림자 말고 실전으로 연결하라고"] 상승보유 이식.
#   근거 실측: 8/19 돈맥 6왕복 중 5건이 팔고 더 올랐다(서진시스템 +5.61% 익절 후 +6.27% 추가).
#   전략 공통모듈(ma3_common: 3분봉 5/10/20 지지 + 수급 우위)이 참이면 이익실현 계열
#   매도(꼭지·che되돌림·시가저항)만 보류한다. 보호장치(1차마감·구조붕괴·재난·하드컷·EOD)는
#   그대로. import 실패 = 기능 통째 꺼짐(fail-open, 종전과 동일하게 판다).
#   끄기: setx MF_MA3_RIDER NO  ·  롤백: backup\money_flow_exec_v1_20260819_before_ma3_rider.py
try:
    from ma3_common_v1 import buy_side_alive as _ma3_buy_side_alive
    from ma3_common_v1 import rider_permit as _ma3_rider_permit
    MA3_RIDER_ON = os.environ.get("MF_MA3_RIDER", "YES").strip().upper() == "YES"
...
def _money_cums(code):
    """[MA3-RIDER] 누적 매수/매도 대금(스냅샷). 없으면 None."""
    try:
        snap = json.loads(Path(r"C:\stock_bot\IPC\live_micro_snapshot.json").read_text(encoding="utf-8-sig"))
        rec = snap.get("codes", {}).get(str(code).zfill(6)) or {}
        b, s = rec.get("buy_money_cum"), rec.get("sell_money_cum")
...
_MA3_HOLD_SEEN = {}   # [MA3-RIDER] 보류 로그 도배 방지(종목당 5분에 1회)


def _ma3_rider_hold(c, p, cur, buy):
    """[MA3-RIDER] 상승보유 허가? — 공용 rider_permit(①선 지지 AND ②수급 우위).

    ②수급 우위는 공용 buy_side_alive(대금속도 10s/30s)를 그대로 쓰되, 속도의 재료는
    스냅샷 누적 대금을 포지션에 표본으로 쌓아 만든다(프로세스 55초 재기동을 넘어
    이어지도록 상태파일에 함께 저장). 표본 부족/자료 없음 = False(보류 안 함 = 판다).
    수익 중일 때만 부른다(호출측 보장) — 손실 구간 보호장치는 이 함수와 무관.
    """
    if not MA3_RIDER_ON:
        return False
    try:
        ...
        if not cums:
            return False
        now_t = time.time()
        hist = p.setdefault("ma3_flow", [])
        hist.append([round(now_t, 2), cums[0], cums[1]])
        while hist and now_t - hist[0][0] > 45:
            hist.pop(0)

        def _rate(win):
            target = now_t - win
            best = min(hist, key=lambda s: abs(s[0] - target))
            dt = now_t - best[0]
            if dt < win * 0.5 or dt > win * 2:
                return None
            return (cums[0] - best[1]) / dt, (cums[1] - best[2]) / dt
        r10, r30 = _rate(10), _rate(30)
        if not r10 or not r30:
            return False
        side = _ma3_buy_side_alive(r10[0], r30[0], r10[1], r30[1])
        held = bool(_ma3_rider_permit(c, cur, buy_side=side))
        if held:
            slot = int(now_t // 300)
            if _MA3_HOLD_SEEN.get(c) != slot:
                _MA3_HOLD_SEEN[c] = slot
                _log(f"🏇 {c} 상승보유 — 5/10/20 지지+수급 우위, 이익실현 매도 보류"
                     f" (수익 {(cur / buy - 1) * 100:+.2f}%)")
        return held
    except Exception:                              # noqa: BLE001
        return False
...
def _exits(bc, pos, hm, mode):
    ...
    for c, p in list(pos.items()):
        ...
        # ★[MA3-RIDER 2026-08-19] 상승보유 판정 — 수익 중일 때만. 아래 사다리에서
        #   이익실현 3계열(che되돌림·꼭지·시가저항)만 이 값으로 보류한다.
        #   1차마감·구조붕괴·재난손절·하드컷은 이 값을 보지 않는다(보호장치 무간섭).
        _rider_hold = (buy > 0 and cur > buy and _ma3_rider_hold(c, p, cur, buy))
        if p.get("ma3_flow"):
            changed = True                       # 수급 표본을 상태파일에 이어 저장
        if (R1_CLOSE_HM and hm >= R1_CLOSE_HM
                and str(p.get("buy_hm", "") or "") <= R1_ENTRY_END
            ...
            sell = f"하드{_hc:g}%" + ("(호가받침연장)" if _hc_ext else "")
              ...
              and not _rider_hold):                                          # [MA3-RIDER] 상승보유 중이면 되돌림 매도 보류
        ...
        elif _top_sell and not _rider_hold:      # [MA3-RIDER] 상승보유 중이면 꼭지 매도 보류
            ...
            sell = _top_sell
              ...
              and not _rider_hold):                                          # [MA3-RIDER] 상승보유 중이면 시가저항 실현 보류

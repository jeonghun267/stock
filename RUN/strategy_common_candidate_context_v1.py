...
HIGH_RANGE_LIVE_PATH = Path(os.environ.get(
    "STRATEGY_CONTEXT_HIGH_RANGE_LIVE",
    str(DATA / "common_high_range_live_state.json")))
...
def _extra_metrics(
    ...
    high_range_live: Mapping[str, Any],
    ...
    if _payload_date(high_range_live, "date", "updated_at") == today:
        for raw_code, row in (high_range_live.get("codes") or {}).items():
            code = _code(raw_code)
            if code and isinstance(row, Mapping):
                extras[code].update({
                    "hr_money_speed_ratio": _num(row.get("money_speed_vs_daily_avg")),
                    "hr_turnover_pct": _num(row.get("listed_turnover_pct")),
...
def build_context(
    *,
    ...
    money_watch: Mapping[str, Any],
    high_range: Mapping[str, Any],
    high_range_live: Mapping[str, Any] | None = None,
    ...
    extras = _extra_metrics(
        today, high_range, high_range_live or {}, selector, board or {})
...
def publish_once(
    ...
    *,
    ...
    shared, valley, audit = build_context(
        ...
        high_range_live=_read_json(HIGH_RANGE_LIVE_PATH),

# -*- coding: utf-8 -*-
# ★[SHADOW-B 2026-08-05 / 2026-08-27 리베이스] 그림자 전용 사본 — 주문 없음, 실전과 무관.
#   현재 실전 strategy_02_low_buy_signal_v1.py 에서 SIX 경로의 수급 관문
#   3개(수급역전·가속·체결량/체결강도)만 통과시켜 "가격만" 판정을 만든다.
#   수급 값 계산과 기록은 그대로 남겨 비교에 쓴다.
#   호가 품질 관문(스프레드·마이크로프라이스·최우선매수비중)은 건드리지 않았다.
#   ⚠️2026-08-27 재작성 이유: 8/05 사본이 그대로 굳어 실전(124KB)과 47KB 벌어져,
#     그림자가 재는 것이 "관문 차이"가 아니라 "옛 코드 차이"가 돼 있었다.
...
class LowBuySignalMonitor:
    ...
    def _detect_six_staircase(
        ...
        if False:  # [SHADOW-B] 가격만 판정 - 수급역전/가속 관문 통과
            state.six_micro_confirm_hits = 0
            state.six_micro_last_confirm_ts = None
            return None
        ...
        if relative is None:
            ...
            #   단, 아래에서 relative[...] 를 읽으므로 통과값으로 채운다.

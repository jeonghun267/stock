...
STATE_SCHEMA = "strategy_common_foundation_v1"
...
class SafetyLimits:
    ...
    def __post_init__(self) -> None:
        if self.fixed_buy_quantity != 1:
            raise ContractError("Strategy common foundation requires exactly one-share buys")
        if self.max_slots != 6:
            raise ContractError("Strategy common foundation requires exactly six slots")
        if as_decimal(self.max_active_capital_krw) != Decimal("2000000"):
            raise ContractError("Strategy common foundation requires 2,000,000 KRW active capital")
...
class SafetyGate:
    ...
    def check_buy(
        ...
        if self.off_flag_path.exists():
            return GateDecision(False, "STRATEGY_OFF_FLAG")

...
# ★[2026-08-13 친구님 지시 "5개 하지 말고 5일평균 10% 이상 되는 것으로"]
#   상위 5종목 자르기를 없애고, 5일 평균 고저폭 문턱으로 대상을 고른다.
#   주문 경로 0(그림자)이라 종목 수가 늘어도 실전 위험은 없다.
#   기존 품질 관문(최근 5일 중 4일 충족)은 그대로 둔다 — 지시에 없던 완화 금지.
#   되돌리기(이 파일은 git 미추적이라 백업본이 없다 — 옛 두 줄을 여기 남긴다):
#     TOP_N = 5
#     return sorted(eligible, key=lambda row: int(row.get("rank") or 9999))[:TOP_N]
MIN_AVG5_RANGE_PCT = 10.0
MIN_QUALIFIED_5D = 4
...
def select_shadow_universe(payload: dict) -> list[dict]:
    ...
    eligible = [
        ...
        and _number(row.get("avg_5d_range_pct")) >= MIN_AVG5_RANGE_PCT
    ]
    return sorted(eligible, key=lambda row: int(row.get("rank") or 9999))

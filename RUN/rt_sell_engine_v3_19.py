...
def _broker_request_se(req_type: str, extra: dict = None,
    ...
    if extra:
        req.update(extra)
    # ★[SIGN-SCOPE 2026-08-14 친구님 지시 "잘못됐으면 고쳐"] 보호 대상일 때만 서명.
    #   8/11 08:56 TR-ROLLBACK 으로 ACCOUNT_INFO·BALANCE_TR 이 PROTECTED_TYPES 에서
    #   빠졌는데 여기는 계속 서명을 시도했다 → sign_order_request 가
    #   "this request type does not use IPC authentication" 예외 → except 가 조용히
    #   None 을 돌려줘 계좌 조회가 무음 실패했다(8/10 지시서의 "매도엔진 계좌 조회가
    #   끊긴다" 경고가 이것). 같은 결함이 1분봉 수집기에도 있어 그쪽은 8/12부터
    #   1분봉 수집을 전량 차단했다.
    #   정상 패턴은 broker_client.py:230-239.
    #   되돌리기: rt_sell_engine_v3_19_20260814_before_sign_scope.py
    if req_type in ("ACCOUNT_INFO", "BALANCE_TR"):
        try:
            from ipc_order_auth_v1 import PROTECTED_TYPES, sign_order_request
            if req_type in PROTECTED_TYPES:

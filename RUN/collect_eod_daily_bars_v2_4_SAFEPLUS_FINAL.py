...
if not log.handlers:
    ...
    try:
        ...
        # Windows에서 이전 프로세스/뷰어가 운영 로그를 잠가도 수집 자체는 계속한다.
        _fallback_log = LOG_DIR / (
            "collect_eod_daily_bars_recovery_" + datetime.now().strftime("%Y%m%d_%H%M%S") + ".log"
        )
        _fh = RotatingFileHandler(
            str(_fallback_log), maxBytes=5 * 1024 * 1024, backupCount=2, encoding="utf-8"
        )
        log.warning("[LOG-FALLBACK] 운영 로그 잠김 — 대체 로그 사용: %s", _fallback_log)
    _fh.setFormatter(_fmt)
    log.addHandler(_fh)


def _set_system_sleep_block(enabled: bool) -> bool:
    """Keep Windows awake while this collector owns an active EOD run."""
...
def _process_created_100ns(pid: int):
    """Return the Windows process creation FILETIME, or None when not alive/readable."""
...
def _legacy_pid_is_same_collector(pid: int, live_created_100ns) -> bool:
    """Validate a legacy integer-only lock using its last heartbeat."""
    if live_created_100ns is None:
        return False
    if int(pid) == os.getpid():
        # acquire 전의 현재 프로세스가 이미 자기 PID를 기록했을 수는 없다.
        return False
    try:
        hb = json.loads(HB_FILE.read_text(encoding="utf-8"))
        if int(hb.get("pid", -1)) != int(pid):
            return True  # 근거 불일치 시 중복 기동보다 차단이 안전하다.
        hb_ts = datetime.fromisoformat(str(hb["ts"]))
        process_started_ts = (int(live_created_100ns) / 10_000_000) - 11_644_473_600
        # 프로세스가 마지막 heartbeat 뒤에 시작했다면 같은 PID를 재사용한 다른 프로세스다.
        return process_started_ts <= hb_ts.timestamp() + 1.0
    except Exception:
        return True
...
def _acquire_pid() -> bool:
    ...
    """
    global _pid_acquired, _pid_token
    if PID_FILE.exists():
        try:
            raw = PID_FILE.read_text(encoding="utf-8").strip()
            try:
                lock_info = json.loads(raw)
                existing_pid = int(lock_info["pid"])
                locked_created = int(lock_info.get("process_created_100ns") or 0)
                legacy_lock = False
            except (ValueError, TypeError, KeyError, json.JSONDecodeError):
                # 이전 버전의 숫자-only PID 파일은 보수적으로 처리한다.
                existing_pid = int(raw)
                locked_created = 0
                legacy_lock = True

            live_created = _process_created_100ns(existing_pid)
            if legacy_lock:
                same_process = _legacy_pid_is_same_collector(existing_pid, live_created)
            else:
                same_process = live_created is not None and locked_created == live_created
            if same_process:
                log.error(f"[PID] 이미 실행 중 (PID={existing_pid}) — 중복 실행 차단")
                return False
            log.warning(
                f"[PID] stale lock 자동 정리 PID={existing_pid} "
                f"(saved_created={locked_created}, live_created={live_created})"
            )
    ...
    _pid_token = f"{os.getpid()}-{time.time_ns()}"
    lock_info = {
        "pid": os.getpid(),
        "process_created_100ns": _process_created_100ns(os.getpid()),
        "token": _pid_token,
    ...
    try:
        ...
        log.error("[PID] 선점 경합 감지 — 중복 실행 차단")
        return False
    _pid_acquired = True
...
def _release_pid() -> None:
    ...
    try:
        ...
        if info.get("token") == _pid_token:
            PID_FILE.unlink(missing_ok=True)
        else:
            log.error("[LOCK][FAIL] PID 파일 소유권 변경 감지 — 삭제하지 않음")
    except Exception as e:
...
class BrokerIpcUnhealthyError(RuntimeError):
    """Raised when the broker heartbeat is fresh but TR responses are jammed."""
...
class BrokerKiwoom:
    ...
    def __init__(self):
        self._consecutive_tr_timeouts = 0
        ...
        for page in range(1, 20):  # 안전 가드 (최대 20페이지)
            ...
            if res.get("status") != "OK":
                ...
                if res.get("status") == "TIMEOUT":
                    self._consecutive_tr_timeouts += 1
                    if self._consecutive_tr_timeouts >= 3:
                        raise BrokerIpcUnhealthyError(
                            "broker IPC TR unhealthy: 3 consecutive opt10081 timeouts"
                        )
                else:
                    self._consecutive_tr_timeouts = 0
                break
...
def _collect_codes(
    ...
    for idx, (code, market) in enumerate(code_market_pairs, start=1):
        ...
        except BrokerIpcUnhealthyError:
            log.error("[BROKER-IPC] TR jam detected; aborting collection immediately")
            raise
...
def main() -> None:
    ...
    sleep_blocked = _set_system_sleep_block(True)
    log.info(f"[POWER] system sleep block={'ON' if sleep_blocked else 'FAILED'}")

...
def _broker_request_bu(req_type: str, extra: dict = None,
    ...
    if extra:
        req.update(extra)
    # ★[SIGN-SCOPE 2026-08-14] 보호 대상일 때만 서명.
    #   rt_sell_engine_v3_19.py 와 동일한 결함이었다 — 8/11 TR-ROLLBACK 이후
    #   ACCOUNT_INFO·BALANCE_TR 은 PROTECTED_TYPES 에 없어 서명 시도가 예외를 냈고,
    #   except 가 조용히 None 을 돌려줬다. 정상 패턴은 broker_client.py:230-239.
    #   되돌리기: kiwoom_buy_order_sender_v4_9_20260814_before_sign_scope.py

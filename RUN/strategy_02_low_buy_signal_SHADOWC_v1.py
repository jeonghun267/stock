# -*- coding: utf-8 -*-
# ★[SHADOW-C 2026-08-05 / 2026-08-27 리베이스] 그림자 전용 사본 — 주문 없음, 실전과 무관.
#   낙폭 구간별 반등 문턱 사다리만 현재 실전과 다르다. 나머지는 동일.
#   ⚠️2026-08-27 재작성 이유는 SHADOW-B 와 같다(옛 사본이 굳어 비교가 오염).
#     옛 사본은 RUNackup\shadow_bc_20260805_stale\ 에 보존.
...
# S02/S06 entry split: a day-low decline of 8% or more from open belongs to S06.
# Candidate selection and signal evaluation stay unchanged; only S02 buy emission is blocked.
S02_S06_SPLIT_DROP_PCT = 8.0
...
class LowBuySignalMonitor:
    ...
    def process_point(
        ...
        *,
        ...
        s06_style_drop_pct = (
            (state.open_price - state.session_low) / state.open_price * 100.0
            if state.open_price > 0 and state.session_low > 0 else None
        )
        if (
            s06_style_drop_pct is not None
            and s06_style_drop_pct >= S02_S06_SPLIT_DROP_PCT
        ):
            self._clear_pending(state)
            return self._wait_row(
                code, name, point, "S02_DROP_GTE_8PCT_HANDOFF_S06"
            ), False

r"""자료원 조기 경보 — 장전 3회 + eod(17:10). 읽기 전용.
...
EOD_DONE = ROOT / "LOG" / "collect_eod.done"
EOD_HEARTBEAT = ROOT / "LOG" / "collect_eod.heartbeat"
BROKER_JOURNAL = ROOT / "LOG" / "broker_journal.log"
...
PHASE_WINDOWS = {
    ...
    "eod": (day_time(16, 55), day_time(20, 30)),
}
# 단계별 재확인 마감.
PHASE_DEADLINES = {
...
RETRY_SEC = 0.25
EOD_HEARTBEAT_MAX_AGE_SEC = 180
EOD_JOURNAL_WINDOW_MIN = 10
EOD_TIMEOUT_MIN_SAMPLES = 20
EOD_TIMEOUT_FAIL_RATIO = 0.50
EOD_DELAYED_CALLBACK_FAIL_COUNT = 30
...
def _read_tail_lines(path: Path, max_bytes: int = 8 * 1024 * 1024) -> tuple[list[str], str]:
    """큰 저널의 끝부분만 읽는다. 브로커 조회나 파일 변경은 하지 않는다."""
    try:
...
_DELAYED_GOT = re.compile(r"got=\(([^,]+),([^,]+),([^\)]+)\)")


def check_eod_health(
    now: datetime,
    done: dict[str, Any],
    heartbeat: dict[str, Any],
    journal_lines: list[str],
) -> list[Result]:
    """일봉 진행 정지·TR 타임아웃 폭증·동일 화면 지연응답 반복을 판정한다."""
    results: list[Result] = []
    today = now.strftime("%Y-%m-%d")
    done_at = str(done.get("done_at") or "")
    try:
        done_qa = float(done.get("qa_score") or 0)
    except (TypeError, ValueError):
        done_qa = 0.0
    done_codes_ok, done_codes = _as_int(done.get("codes"))
    done_ok = (
        done_at.startswith(today)
        and str(done.get("status") or "").upper() == "OK"
        and done_qa >= 90.0
        and done_codes_ok and done_codes > 0
    )
    if done_ok:
        results.append((PASS, "일봉 완료", f"done_at={done_at} codes={done.get('codes')}"))
    else:
        results.append((INFO, "일봉 완료", "아직 완료 신호 없음 — 진행 상태를 검사한다"))

    if done_ok:
        results.append((PASS, "일봉 진행", "완료 신호가 있어 heartbeat 노화는 정상"))
    else:
        hb_ts = str(heartbeat.get("ts") or "")
        age = _age_seconds(now, hb_ts)
        ok_idx, idx = _as_int(heartbeat.get("idx"))
        ok_total, total = _as_int(heartbeat.get("total"))
        if not hb_ts.startswith(today) or not ok_idx or not ok_total or total <= 0:
            results.append((FAIL, "일봉 진행", "오늘의 정상 heartbeat를 확인할 수 없음"))
        elif age > EOD_HEARTBEAT_MAX_AGE_SEC:
            results.append((FAIL, "일봉 진행 정지",
                            f"idx={idx}/{total}, heartbeat age={age:.0f}초 > "
                            f"{EOD_HEARTBEAT_MAX_AGE_SEC}초"))
        else:
            results.append((PASS, "일봉 진행",
                            f"idx={idx}/{total}, heartbeat age={age:.0f}초"))

    cutoff = now - timedelta(minutes=EOD_JOURNAL_WINDOW_MIN)
    ok_count = timeout_count = 0
    delayed: Counter[str] = Counter()
    for line in journal_lines:
        ...
        try:
            ...
            continue
        if stamp < cutoff or stamp > now + timedelta(seconds=5):
            continue
        if " RES " in line:
            if "status=TIMEOUT" in line:
                ...
                ok_count += 1
        if "지연 응답 무시" in line:
            got = _DELAYED_GOT.search(line)
            delayed[got.group(0) if got else "signature-unparsed"] += 1

    samples = ok_count + timeout_count
    ratio = timeout_count / samples if samples else 0.0
    if samples < EOD_TIMEOUT_MIN_SAMPLES:
        results.append((INFO, "브로커 TIMEOUT 비율",
                        f"최근 {EOD_JOURNAL_WINDOW_MIN}분 표본 {samples}건 < "
                        f"{EOD_TIMEOUT_MIN_SAMPLES}건"))
    elif ratio >= EOD_TIMEOUT_FAIL_RATIO:
        results.append((FAIL, "브로커 TIMEOUT 비율",
                        f"{timeout_count}/{samples}={ratio:.1%} >= "
                        f"{EOD_TIMEOUT_FAIL_RATIO:.0%}"))
    else:
        results.append((PASS, "브로커 TIMEOUT 비율",
                        f"{timeout_count}/{samples}={ratio:.1%}"))

    signature, leak_count = delayed.most_common(1)[0] if delayed else ("", 0)
    if leak_count >= EOD_DELAYED_CALLBACK_FAIL_COUNT:
        results.append((FAIL, "TR 화면 지연응답 반복",
                        f"동일 {signature} {leak_count}회/{EOD_JOURNAL_WINDOW_MIN}분"))
    else:
        results.append((PASS, "TR 화면 지연응답 반복",
                        f"최대 {leak_count}회/{EOD_JOURNAL_WINDOW_MIN}분"))
    return results
    ...
    if fails and "--no-popup" not in sys.argv:
        try:
            ...
            if phase == "live":
                ...
                tail = "일봉 수집과 대량 TR 충돌을 확인해야 합니다. 자동 조치는 하지 않습니다."
            else:
                tail = "08:59 정규 사전점검 전에 조치가 필요합니다."
...
def main() -> int:
    ...
    if phase not in PHASE_WINDOWS:
        print("--phase board|preopen|live|eod 를 명시해야 한다"
              f" (받은 값: {phase or '(없음)'})")
        return 2
    ...
    if phase == "eod":
        done, _ = _read_json(EOD_DONE)
        heartbeat, _ = _read_json(EOD_HEARTBEAT)
        journal_lines, journal_error = _read_tail_lines(BROKER_JOURNAL)
        results = notes + check_eod_health(now, done, heartbeat, journal_lines)
        if journal_error:
            results.append((FAIL, "브로커 저널", journal_error))
        return _emit(now, phase, results)

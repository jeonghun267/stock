# -*- coding: utf-8 -*-
"""C2-01 order-zero signal to Captain2 order-path bridge.
...
STRATEGY_NUMBER = "01"
STRATEGY_ID = "C2_01_OPEN_SURGE"
STRATEGY_NAME = "장초반 급상승 초입"
SIGNAL_SCHEMA = "captain2_c2_01_shadow_v1"
SIGNAL_MODE = "SHADOW_ORDER_ZERO"
...
def _signal_id(day: str, row: Mapping[str, Any]) -> str:
    return f"{day}:{str(row.get('code') or '').zfill(6)}:{row.get('ts')}"
...
def select_fresh_signals(
    ...
    *,
    ...
    """Return fresh, unique C2-01 BUY_READY rows in deterministic priority order."""

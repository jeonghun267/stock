...
# ★[TR-ROLLBACK 2026-08-11 08:56 친구님 승인 "TR 4종 빼고 재기동"] TR·BATCH_TR·
#   ACCOUNT_INFO·BALANCE_TR 를 뺐다. 오늘 08:35 브로커 재기동으로 이 코드가 처음
#   실전에 뜨자 08:40:01 부터 조회가 전량 거부됐다(어제 0건 → 오늘 747건,
#   caller= 빈칸 = BrokerClient 를 안 거치고 IPC json 을 직접 쓰는 호출자).
#   위 51~54줄이 이미 경고한 그대로다 — 직접 쓰는 호출자는 서명이 붙지 않는다.
#   8/10 지시서 대비: 시킬 것은 BATCH_TR 1건뿐이었고 ACCOUNT_INFO 는 명시적 금지,
#   TR 은 "사용량 실측 먼저" 였다. 되살리려면 호출자 전수를 BrokerClient 경유로
#   바꾼 뒤 한 종류씩. 실주문(SENDORDER_REAL)·SHUTDOWN 서명은 그대로 유지된다.
PROTECTED_TYPES = frozenset({
    "SENDORDER_REAL",
    "SET_REAL_REMOVE_ALL",
    "SET_REAL_REMOVE",      # [2026-08-07] 형태 무관 전부 강제(종전엔 ALL 일 때만)
    "SETREAL_REG",
...
class NonceStore:
    ...
    def __init__(
        self,
        ttl_sec: float = MAX_AUTH_AGE_SEC * 2,
        *,
    ...
    def _load(self) -> bool:
        ...
        try:
            ...
            for nonce, raw_ts in raw_seen.items():
                if not _NONCE_RE.fullmatch(str(nonce or "")):
                    return False
                ts = float(raw_ts)
                if not (ts == ts and abs(ts) != float("inf")):
                    return False
                restored[str(nonce)] = ts
            self._seen = restored
            return True
    ...
    def _save(self) -> bool:
        ...
        try:
            ...
            tmp.write_text(
                json.dumps({"seen": self._seen, "updated_at": time.time()}),
                encoding="utf-8",
            )

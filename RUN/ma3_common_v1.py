...
def three_minute_closes(code: str, payload: Dict[str, Any]) -> List[float]:
    ...
    prev 는 완성 1분봉 [o,h,l,c] 의 오래된→최근 배열이고 시각이 없다.
    payload["hm"] 이 '진행 중인 분'이므로, prev[i] 의 분 = 현재분 - (len-i) 로
    역산해 3분 격자(09:00~09:02, 09:03~09:05 …)에 담는다.
    · 1분봉 3개가 다 있는 블록만 인정한다(결손 블록은 버린다).

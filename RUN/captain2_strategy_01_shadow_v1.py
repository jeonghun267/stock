...
from captain2_strategy_01_open_surge_v1 import (
    BuyAction,
...
class ShadowConfig:
    watch_path: Path = Path(os.environ.get(
        "C2_01_WATCH", r"C:\stock_bot\IPC\micro_watch_captain2.json"))
    snapshot_path: Path = Path(os.environ.get(
        "C2_01_SNAPSHOT", r"C:\stock_bot\IPC\live_micro_snapshot.json"))
    board_path: Path = Path(os.environ.get(
        "C2_01_MONEY_BOARD", r"C:\stock_bot\data\micro_rank_board.json"))
    minute_path: Path = Path(os.environ.get(
        "C2_01_MINUTE", r"C:\stock_bot\data\돈맥_1분봉.json"))
    name_path: Path = Path(os.environ.get(
        "C2_01_NAMES", r"C:\stock_bot\data\_code_name_cache.json"))
    output_path: Path = Path(os.environ.get(
        "C2_01_OUTPUT", r"C:\stock_bot\data\captain2_c2_01_shadow.json"))
    event_dir: Path = Path(os.environ.get(
        "C2_01_EVENT_DIR", r"C:\stock_bot\data\shadow"))
    html_path: Path = Path(os.environ.get(
        "C2_01_HTML", r"C:\Users\UserK\Desktop\캡틴2_C2-01_급상승_모의감시.html"))
    loop_sec: float = float(os.environ.get("C2_01_LOOP_SEC", "1.0"))
    max_snapshot_age_sec: float = float(os.environ.get("C2_01_SNAPSHOT_MAX_AGE", "5"))
    max_board_age_sec: float = float(os.environ.get("C2_01_BOARD_MAX_AGE", "10"))
...
<meta http-equiv="refresh" content="2"><title>C2-01 급상승 모의감시</title>
...
<h1>캡틴2 C2-01 장초반 급상승 — 주문 0 모의감시</h1>
...
class ShadowRuntime:
    ...
    def tick(self, now: datetime) -> Dict[str, Any]:
        ...
        event_path = self.config.event_dir / f"captain2_c2_01_shadow_{now:%Y%m%d}.csv"
